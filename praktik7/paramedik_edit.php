...
$periksa_id = $_GET['id'];
$paramedik = $dbh->query("SELECT * FROM paramedik WHERE id = $periksa_id")->fetch();
...
<div class="container-fluid px-4">
    ...
    <input type="hidden" value="<?= $paramedik['id'] ?>" name="id">
        ...
        <div class="form-group row">
            ...
            <div class="col-8">
                <div class="custom-control custom-radio custom-control-inline">
                    <input name="gender" id="gender_0" type="radio" class="custom-control-input" value="M" <?= $paramedik['gender'] == 'M'?'checked': ''?>>
                    <label for="gender_0" class="custom-control-label">Laki-Laki</label>
                </div>
                <div class="custom-control custom-radio custom-control-inline">
                    <input name="gender" id="gender_1" type="radio" class="custom-control-input" value="F" <?= $paramedik['gender'] == 'F'?'checked': ''?>>
            ...
            <div class="col-8">
                <select id="unit_kerja" name="unit_kerja" class="custom-select">
                    ...
                    <?php foreach ($unit_kerja as $unit) : ?>
                        <option value="<?= $unit['id'] ?>" <?= $paramedik['unit_kerja_id'] == $unit['id']?'selected': ''?>><?= $unit['nama'] ?></option>
                    <?php endforeach ?>
                </select>
            </div>
        </div>
        <div class="form-group row">
            ...
            <div class="col-8">
                <select id="kategori" name="kategori" class="custom-select">
                    ...
                    <option value="poli umum" <?= $paramedik['kategori'] == 'poli umum'?'selected': ''?>>Poli Umum</option>
                    <option value="poli gizi" <?= $paramedik['kategori'] == 'poli gizi'?'selected': ''?>>Poli Gizi</option>
                    <option value="poli anak" <?= $paramedik['kategori'] == 'poli anak'?'selected': ''?>>Poli Anak</option>
